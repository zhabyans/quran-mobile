...
const ListSurah = (props) => {
  ...
  const getDataFromNetwork = async () => {
    try {
      let response = await Axios.get('https://equran.id/api/surat', {
        timeout: 2000,
      });
      setIsReady(true);
  ...
  const getDataStorageToRedux = async () => {
    try {
      ...
      jsonValue != null
        ? props.ayatTerakhirHandle(JSON.parse(jsonValue))
        : null;
    } catch (e) {
      alert(e);
    }
    ...
    render() {
      return (
        <>
          <TouchableOpacity
            onPress={() =>
              props.navigation.push('DetailSurah', {
                pesan: this.isi,
                screen: 'ayatPilihan',
                surahNumber: this.isi.nomor,
              })
            }>
              ...
              <View style={styles.containerRow}>
                ...
                <View style={styles.surahRow}>
                  ...
                  <Text style={styles.surahDetail}>
                    {this.isi.tempat_turun} - {this.isi.jumlah_ayat} AYAT
                  </Text>
                </View>
        ...
        <View style={styles.containerHome}>
          <TouchableOpacity
            ...
            <View style={styles.lastReadContainer}>
              <Image
                style={styles.lastSurahImage}
                source={require('../../assets/images/last-surah.png')}
              />
              <View style={styles.lastReadContainerText}>
                <View style={styles.lastReadRow}>
                  <Image
                    source={{
                      uri:
                        'https://img.icons8.com/color/96/000000/open-book.png',
                    }}
                    style={{width: 22, height: 22, marginRight: 10}}
                  />
                  <Text style={styles.lastReadText}>Terakhir Dibaca</Text>
                </View>
                <Text style={styles.surahTerakhir}>
                  {props.ayatTerakhir == null
                    ? 'Belum Dibaca'
                    : props.ayatTerakhir.nama_latin}
                </Text>
                <Text style={styles.ayah}>
                  {props.ayatTerakhir == null
                    ? ''
                    : 'Ayat ke - ' + props.ayatTerakhir.ayat}
                </Text>
              </View>
            </View>
          </TouchableOpacity>
      ...
      <>
        {isLoading ? (
          ...
          <FlatList
            ...
            renderItem={({item}) => <ListItemRender isi={item} />}
...
const mapStateToProps = (nilai) => {
  return {
    ayatTerakhir: nilai.userReducer.ayatTerakhir,
  };
};
