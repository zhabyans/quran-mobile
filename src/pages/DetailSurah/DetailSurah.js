...
const DetailSurah = (props) => {
    ...
    //     try {
    //         let response = await Axios.get('https://equran.id/api/surat/' + props.route.params.pesan.nomor);
    //         setLoading(false)
    //     } catch (error) {
    //         console.log(error)
    //     }
        ...
        <>
            ...
            <>
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <ListItemRender isi={item} kiriman={{ nama_latin: props.route.params.pesan.nama_latin, nomor: props.route.params.pesan.nomor }} />
                    )}
                    ref={flatList}
                    onLayout={() => {
                        if (props.route.params.screen == 'ayatTerakhir') {
                            flatList.current.scrollToIndex({ index: props.route.params.pesan.ayat - 1, viewPosition: 0.5 })
                        }
                    }}
                />
            </>
